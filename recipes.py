ingredients = {
    ...
    'Dunkel': {
        'Солод': [['Pils', 215], ['Munhen 25', 25], ['Roasted', 15]],
        'Хмель': [['Perle', 0.35], ['Mandarina', 0.35], ['Mitlfruh', 0.5]]
    },
    'Dunkel_1-3': {
...
def count_materials(beer_name, number_tank,):
    ...
    return format_result(beer_name, nums=nums)
...
def count_ingridients_for_tank(beer_name, dict):
    answer = f"<b>Сырьё для {beer_name}:</b>\n"
...
def format_result(beer_name, nums=1):
    dict_ingridients = create_dict_ingridients(beer_name, nums=1)
    if beer_name == 'Dunkel_1-3':
        beer_name == 'Dunkel'
    if nums > 1:
        answer = count_ingridients_for_tank(beer_name, dict_ingridients)
    else:
